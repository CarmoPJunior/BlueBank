...
public interface EnderecoControllerDocs {
	...
	ResponseEntity<EnderecoDTO> update(@PathVariable Integer id, @RequestBody EnderecoDTO obj) throws EnderecoNaoEncontradoException;
	
	@ApiOperation(value = "Cria um novo endereço")
    @ApiResponses(value = {
            @ApiResponse(code = 201, message = "Endereço criado!"),
            @ApiResponse(code = 400, message = "Requisição possui campos inválidos!")
