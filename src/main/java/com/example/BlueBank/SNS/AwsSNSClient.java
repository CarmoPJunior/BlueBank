...
public class AwsSNSClient {
	...
	public static void sendSMS(String messege, String phoneNumber) {
		...
		smsAttributes.put("AWS.SNS.SMS.SenderID",
				new MessageAttributeValue().withStringValue("BlueBank").withDataType("String"));

		smsAttributes.put("AWS.SNS.SMS.SMSType",
				new MessageAttributeValue().withStringValue("Transactional").withDataType("String"));

		PublishResult result = snsClient.publish(new PublishRequest().withMessage(messege).withPhoneNumber(phoneNumber)
	...
	public static void sendToTopic(String subject, String message) {
		...
		PublishResult result = snsClient
				.publish(new PublishRequest().withTopicArn("arn:aws:sns:us-east-1:965934840569:UndefinedCoders")
						.withMessage(message).withSubject(subject).withMessageAttributes(smsAttributes));
