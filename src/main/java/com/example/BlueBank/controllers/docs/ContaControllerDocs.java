...
public interface ContaControllerDocs {
	...
	@ApiResponses(value = { @ApiResponse(code = 200, message = "Conta encontrada!"),
			@ApiResponse(code = 404, message = "Conta não encontrado!") })
	ResponseEntity<Page<TransacaoDTO>> findById(@PathVariable Integer id, Pageable page)
	...
	@ApiResponses(value = { @ApiResponse(code = 200, message = "Conta atualizada com sucesso!"),
			@ApiResponse(code = 400, message = "Requisição possui campos inválidos!"),
			@ApiResponse(code = 404, message = "Conta não encontrada!") })
	ResponseEntity<ContaDTO> update(@PathVariable Integer id, @RequestBody ContaDTO obj)
			throws ContaNaoEncontradaException;

	@ApiOperation(value = "Atualiza o Status de uma conta, passando o ID da Conta o estado a ser alterado")
	@ApiResponses(value = { @ApiResponse(code = 200, message = "Conta bloqueada com sucesso!"),
			@ApiResponse(code = 400, message = "Requisição possui campos inválidos!"),
			@ApiResponse(code = 401, message = "Não foi possível fazer a operação. É preciso estar zerada a conta!"),
			@ApiResponse(code = 404, message = "Conta não encontrada!") })
	ResponseEntity<ContaDTO> updateStatus(@PathVariable Integer id, @RequestBody ContaDTO obj)
	...
	@ApiResponses(value = { @ApiResponse(code = 200, message = "Conta cadastrada com sucesso!"),
			@ApiResponse(code = 400, message = "Requisição possui campos inválidos!"),
			@ApiResponse(code = 404, message = "Conta não encontrada!") })
	ResponseEntity<ContaDTO> create(@RequestBody @Valid Conta obj)
	...
	@ApiResponses(value = { @ApiResponse(code = 200, message = "Conta deletada com sucesso!"),
			@ApiResponse(code = 400, message = "Requisição possui campos inválidos!"),
			@ApiResponse(code = 404, message = "Conta não encontrada!") })
