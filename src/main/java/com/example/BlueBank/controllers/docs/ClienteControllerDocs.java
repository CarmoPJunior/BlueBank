...
public interface ClienteControllerDocs {
	...
	@ApiResponses(value = { @ApiResponse(code = 200, message = "Lista todos os clientes cadastrados."), })
	...
	@ApiResponses(value = { @ApiResponse(code = 200, message = "Cliente atualizado com sucesso!"),
			@ApiResponse(code = 400, message = "Requisição possui campos inválidos!"),
			@ApiResponse(code = 404, message = "Cliente não encontrado!") })
	ResponseEntity<ClienteDTO> update(@PathVariable Integer id, @RequestBody @Valid ClienteDTO obj)
			throws ClienteNaoEncontradaException;

	@ApiOperation(value = "Deleta um cliente.")
	@ApiResponses(value = { @ApiResponse(code = 200, message = "Cliente deletado com sucesso!"),
			@ApiResponse(code = 404, message = "Cliente não encontrado!") })
